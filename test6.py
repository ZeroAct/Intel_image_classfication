...
from MyModels import CreateModeld3d
...
model = CreateModeld3d()

hist = model.fit_generator(generator=train_generator,
                           steps_per_epoch=STEP_SIZE_TRAIN,
                           validation_data=validation_generator,
                           validation_steps=STEP_SIZE_VALID,
                           epochs=50, callbacks=[cp_callback, rd_callback])

tmp = hist.history.copy()
del tmp['lr']
js = json.dumps(tmp)
f = open("history_preprocess_freeze_rd_aug_3drop_dense.json","w")
f.write(js)
f.close()
